...
exports.testRoutes = async () => {
  try {
    const responses = await Promise.all([
      fetch(`${process.env.API_URL}/api/user/ping`),
      fetch(`${process.env.API_URL}/api/car/ping`),
      fetch(`${process.env.API_URL}/api/invoice/ping`),
      fetch(`${process.env.API_URL}/api/payment/ping`),
      fetch(`${process.env.API_URL}/api/appointment/ping`),
    ]);

    const data = await Promise.all(responses.map((response) => response.json()));
    return data;
  } catch (error) {
    logger.error(`[testRoutes] ${error}`);
